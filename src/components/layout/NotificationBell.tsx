        ...
        <div className="absolute right-0 top-full mt-2 w-[360px] max-w-[92vw] bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
              ...
              {/* Tahasang DALAWANG pindutan, hindi toggle — may laging kitang
                  daan pabalik sa lahat (iniulat ng may-ari, Ago 24 2026, na
                  hindi siya makabalik matapos pindutin ang Mentions). */}
              <span className="inline-flex rounded-full border border-slate-200 overflow-hidden">
                <button onClick={() => setOnlyMentions(false)}
                  className={`text-[11px] font-bold px-2 py-0.5 ${!onlyMentions ? "bg-slate-800 text-white" : "bg-white text-slate-500 hover:bg-slate-50"}`}>
                  All
                </button>
                <button onClick={() => setOnlyMentions(true)}
                  title="Only notifications where you were mentioned"
                  className={`text-[11px] font-bold px-2 py-0.5 ${onlyMentions ? "bg-indigo-600 text-white" : "bg-white text-indigo-600 hover:bg-indigo-50"}`}>
